...
EdUECMmeansNuc %>% filter(CollagenI != 0) %>% group_by(Abb_ECMs, CollagenI) %>% summarize(Percent_Positive = mean(Percent_Positive)) %>%ggplot( aes(x = Abb_ECMs, y = Percent_Positive, col = CollagenI))+
  ...
  ylab("% EdU Positive")
ggsave("CollagenIRatiovsECMEdU.png")
...
EdUECMmeansNuc %>% filter(Hyaluronic_Acid != 0) %>% group_by(Abb_ECMs, Hyaluronic_Acid) %>% summarize(Percent_Positive = mean(Percent_Positive)) %>%ggplot( aes(x = Abb_ECMs, y = Percent_Positive, col = Hyaluronic_Acid))+
  #facet_grid(cols = vars(Metadata_Stiffness))+
  geom_path(aes(group = Hyaluronic_Acid), linewidth = 1.5)+
  geom_point()+
  theme_prism(border = TRUE)+
  xlab("ECM Composition")+
  ylab("% Positive EdU")+
...
C1TEsig <- compare_means(Mean_Percent_Positive ~ CollagenI, subset(dswithrepAbbECM, CollagenI != 0), group.by = c("Metadata_Treatment", "Abb_ECMs"))

C4TEsig <- compare_means(Mean_Percent_Positive ~ CollagenIV, subset(dswithrepAbbECM, CollagenIV != 0), group.by = c("Metadata_Treatment", "Abb_ECMs"))

HATEsig <-compare_means(Mean_Percent_Positive ~ Hyaluronic_Acid,subset(dswithrepAbbECM, Hyaluronic_Acid != 0), group.by = c("Metadata_Treatment", "Abb_ECMs"))

TCTEsig <-compare_means(Mean_Percent_Positive ~ TenascinC,subset(dswithrepAbbECM, TenascinC != 0), group.by = c("Metadata_Treatment", "Abb_ECMs"))

FNTEsig <-compare_means(Mean_Percent_Positive ~ Fibronectin, subset(dswithrepAbbECM, Fibronectin != 0),group.by = c("Metadata_Treatment", "Abb_ECMs"))
```
...
treatmentcomp <- list(c("DMSO", "10 uM"), c("DMSO", "20 uM"), c("10 uM", "20 uM"))
dswithrepAbbECM %>% ggplot(aes(Metadata_Treatment, Mean_Percent_Positive, color = Metadata_Stiffness)) +
  geom_boxplot()+
  theme_prism()+
  stat_compare_means()
```
